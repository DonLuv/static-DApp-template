...
npm run generate        # Compile + generate typed ABI + hooks per contract (dapp/)
...
│   ├── wagmi.config.ts       # ABI codegen config — auto-discovers contracts from evm/artifacts
│   ├── vite.config.ts
│   ├── tsconfig.json
│   ├── tailwind.config.js
│   ├── postcss.config.js
...
│       │   ├── contracts/
│       │   │   └── counter/
│       │   │       ├── generated.ts  # @wagmi/cli output (git-tracked)
│       │   │       └── config.ts     # Re-exports generated ABI + env address
...
2. `npm run generate` — compiles contracts, then `@wagmi/cli` auto-discovers artifacts and generates per-contract typed ABI exports + React hooks into `dapp/src/lib/contracts/<name>/generated.ts`
3. Import generated hooks in DApp components
...
2. `npm run generate` (compiles first, then auto-discovers the new contract)
3. Create `dapp/src/lib/contracts/<contract-name>/config.ts` — re-export ABI from `./generated` + address from env
4. Create UI components in `dapp/src/components/contracts/<contract-name>/`
5. Add any new env vars (e.g., contract address) following the env var convention
6. Commit the generated file (`dapp/src/lib/contracts/<contract-name>/generated.ts` is git-tracked)

Contracts in `evm/contracts/mocks/` or `evm/contracts/test/` are excluded from codegen by convention.
...
- **per-contract `generated.ts`**: Auto-generated by `npm run generate` (@wagmi/cli) into `dapp/src/lib/contracts/<name>/generated.ts`. Don't edit manually. Git-tracked (commit after regenerating).
...
- **Compile before generate**: `npm run generate` (root script) compiles first automatically. The workspace-level `npm run generate -w dapp` does NOT compile — use the root script or run `npm run compile` first.
